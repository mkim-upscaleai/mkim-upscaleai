    ...
    def start_session(self):
        ...
        if self.use_vtysh or self.duthost.get_frr_mgmt_framework_config():
            ...
            if not self.duthost.get_frr_mgmt_framework_config():
                ...
                is_v6 = ipaddress.ip_address(self.ip).version == 6
                peer_group = "PEER_V6" if is_v6 else "PEER_V4"
                afi_safi = "ipv6_unicast" if is_v6 else "ipv4_unicast"
        ...
        if self.use_vtysh:
            ...
            for asichost in self.duthost.asics:
                if self.duthost.get_frr_mgmt_framework_config():
                    afi = "ipv6_unicast" if ipaddress.ip_address(self.ip).version == 6 else "ipv4_unicast"
                    asichost.run_sonic_db_cli_cmd(
                        "CONFIG_DB del 'BGP_NEIGHBOR|default|{}'".format(self.ip))
                    asichost.run_sonic_db_cli_cmd(
                        "CONFIG_DB del 'BGP_NEIGHBOR_AF|default|{}|{}'".format(self.ip, afi))
                else:
