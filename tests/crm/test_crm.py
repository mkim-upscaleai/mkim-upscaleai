...
def verify_thresholds(duthost, asichost, **kwargs):
    ...
    try:
        ...
        for key, value in list(THR_VERIFY_CMDS.items()):
            ...
            # Disable autoescape: these templates render bash/CLI commands, not HTML,
            # so HTML-escaping would corrupt characters like <, >, &, ', " in resource
            # names or numeric thresholds. Inputs are controlled test constants, so
            # template-injection is not a concern here.
            template = Environment(autoescape=False).from_string(value)
            if "exceeded" in key:
